...
def create_ec2_role():
    ...
    ec2_role_policy = aws.iam.RolePolicy("ec2-role-policy",
...
def create_iam_resources():
    # Create 'dev' user
    dev_user = aws.iam.User("dev-user", name="cobra-scenario-8-dev-user", path="/system/")
    ...
    dev_user_lambda_admin = aws.iam.UserPolicyAttachment("dev-lambda-admin-policy-attachment",
                                                         user=dev_user.name,
    ...
    iam_monitor_role = aws.iam.Role("iam-monitor-role",
    ...
    return {"lambda_role": lambda_role, "iam_monitor_role": iam_monitor_role, "dev_access_key": dev_access_key}


def add_agent_bucket_permission(ec2_role_name, agent_bucket):
    aws.iam.RolePolicy("agent-bucket-read-policy",
                       role=ec2_role_name,
                       policy=pulumi.Output.all(agent_bucket.arn).apply(lambda args: json.dumps({
                           "Version":
                           "2012-10-17",
                           "Statement": [{
                               "Effect": "Allow",
